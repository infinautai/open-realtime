...
from audio.utils import calculate_audio_volume, exp_smoothing
...
class VADAnalyzer(ABC):
    ...
    def analyze_audio(self, buffer, offset) -> Generator[VADState, int, None]:
        self._vad_buffer += buffer
        num_required_bytes = self._vad_frames_num_bytes
        ajusted_offset = offset - len(self._vad_buffer) if offset > len(self._vad_buffer) else 0
        time_offset = ajusted_offset / (self.sample_rate * self._num_channels * 2)
        if len(self._vad_buffer) < num_required_bytes:
            yield (self._vad_state, time_offset)

        while len(self._vad_buffer) >= num_required_bytes:
            audio_frames = self._vad_buffer[:num_required_bytes]
            self._vad_buffer = self._vad_buffer[num_required_bytes:]
            
            time_offset += num_required_bytes / (self.sample_rate * self._num_channels * 2)
            new_state = self._analyze_audio(audio_frames)
            yield (new_state, time_offset)
            
    
    def _analyze_audio(self, audio_frames) -> VADState:
